...
public class ChartUtil {
    ...
    public List<String[]> getCs() {
        return Arrays.asList(new String[]{"OSI 7 계층에서 데이터를 전기적, 광적 신호로 변환하여 전송하는 계층은?", "물리 계층"},
                ...
                new String[]{"#1은 데이터를 처리하고 저장하는 역할을 하며, #2는 사용자 인터페이스를 표시하는 역할을 하며, #3는 #1과 #2를 제어하고 상호작용을 조정하는 역할을 한다.", "Model/View/Controller"},
                ...
                new String[]{"#1은 인덱스를 통해 빠른 접근이 가능하지만 크기가 고정되어 있으며, 삽입, 삭제가 불편하다. #2는 포인터를 통해 다음 원소에 접근하며, 크기가 동적으로 변경이 가능하며 삽입, 삭제가 용이하지만, 인덱스를 통한 접근은 불가능하다.", "배열/리스트"},
