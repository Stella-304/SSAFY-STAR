...
public class CardServiceImpl implements CardService {
	...
	// 	List<CardDetailDto> detailDtoList = setCoordinatesV1(cardList);
	//
	...
	// 		System.out.println(curCardGroupCnt);
	...
	// public List<CardDetailDto> setCoordinatesV1(List<Card> cardList) {
	...
	public ConstellationListDto getCardListV2(SearchConditionReqDto searchConditionReqDto) {
		...
		for (int i = 0; i < groupSize; i++) {
			...
			// 3배 뽑아서 조져버려.
			while (!queue.isEmpty() && choosePosList.size() < willChooseCardCnt * 1.3) {
				int[] cur = queue.poll();
			...
			// curGroupEdgeDtoList = getCurGroupEdgeDtoList(curGroupCardDetailDtoList);
	...
	//
	// public List<EdgeDto> getCurGroupEdgeDtoList(List<CardDetailDto> curGroupCardDetailDtoList) {
	// 	// 컨벡스헐을 이용해서 볼록껍질을 찾자.
	// 	// 나머지 점들은 이어주자.
	//
	// 	double firstZ = 10000;
	...
	// 	for(int i=0; i<curGroupCardDetailDtoList.size(); i++){
	// 		if(curGroupCardDetailDtoList.get(i).getX() < firstX){
	// 			firstX = points.get(i);
